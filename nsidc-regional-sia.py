...
def plotRegionalGraphsAntarctic(filename):
	...
	saveRegionalPlot(2, 0, 6.5, data, "Weddell NSIDC sea ice area", "nsidc-area-weddell.png", 2) #6.3
	saveRegionalPlot(3, 0, 7.5, data, "Weddell NSIDC sea ice extent", "nsidc-extent-weddell.png", 2) # 7.4
	...
	saveRegionalPlot(14, 5, 16, data, "NSIDC Antarctic sea ice area", filenameAntarcticArea, 2)
	saveRegionalPlot(15, 7, 20, data, "NSIDC Antarctic sea ice extent", filenameAntarcticExtent, 2)
	...
	saveRegionalPlot(-3, 0.7, 0.85, data, "NSIDC Antarctic sea ice compactness (area divided by extent)", filenameAntarcticCompactness, 4)
...
def plotRegionalGraphsArctic(filename):
	data = np.loadtxt(filename + ".csv", delimiter=",", dtype=str)
	legendpos = 8
	saveRegionalPlot(2, 2.0, 4.6, data, "Central Arctic NSIDC sea ice area", "nsidc-area-cab.png", 3)
	saveRegionalPlot(3, 3.0, 4.6, data, "Central Arctic NSIDC sea ice extent", "nsidc-extent-cab.png", 3)
	...
	saveRegionalPlot(12, 0, 1, data, "Kara NSIDC sea ice area", "nsidc-area-kara.png", 1)
	saveRegionalPlot(13, 0, 1, data, "Kara NSIDC sea ice extent", "nsidc-extent-kara.png", 3)
	saveRegionalPlot(14, 0, 0.6, data, "Barents NSIDC sea ice area", "nsidc-area-barents.png", 1)
	saveRegionalPlot(15, 0, 0.8, data, "Barents NSIDC sea ice extent", "nsidc-extent-barents.png", 1)
	saveRegionalPlot(16, 0, 0.6, data, "Greenland Sea NSIDC ice area", "nsidc-area-greenland.png", 1)
	saveRegionalPlot(17, 0, 0.8, data, "Greenland Sea NSIDC ice extent", "nsidc-extent-greenland.png", 1)
	saveRegionalPlot(18, 0, 1.0, data, "Baffin Bay NSIDC sea ice area", "nsidc-area-baffin.png", 1)
	saveRegionalPlot(19, 0, 1.4, data, "Baffin Bay NSIDC sea ice extent", "nsidc-extent-baffin.png", 1)
	saveRegionalPlot(22, 0, 1.2, data, "Hudson Bay NSIDC sea ice area", "nsidc-area-hudson.png", 1)
	...
	saveRegionalPlot(26, 0, 0.5, data, "Bering NSIDC sea ice area", "nsidc-area-bering.png", 1)
	saveRegionalPlot(27, 0, 1.0, data, "Bering NSIDC sea ice extent", "nsidc-extent-bering.png", 1)
	saveRegionalPlot(28, 0, 0.4, data, "Okhotsk NSIDC sea ice area", "nsidc-area-okhotsk.png", 1)
	saveRegionalPlot(29, 0, 0.6, data, "Okhotsk NSIDC sea ice extent", "nsidc-extent-okhotsk.png", 1)
	...
	saveRegionalPlot(-3, 0.55, 0.92, data, "NSIDC Arctic sea ice compactness (area divided by extent)", filenameArcticCompactness, 3)
	saveRegionalPlot(-2, 3, 7.4, data, "Arctic Basin NSIDC sea ice extent", filenameArcticBasinExtent, 3)
	saveRegionalPlot(-1, 0, 7.5, data, "Arctic Basin NSIDC sea ice area", filenameArcticBasinArea, 3)
	...
	saveRegionalPlot(41, 0, 12.3, data, "NSIDC Arctic sea ice area", filenameArcticArea, 3)
	saveRegionalPlot(42, 3, 14, data, "NSIDC Arctic sea ice extent", filenameArcticExtent, 3)	
	saveRegionalPlot(41, -2, 0.5, data, "NSIDC Arctic sea ice area anomaly vs. 1990-2019", filenameArcticAreaAnomaly, 3, True)
	saveRegionalPlot(42, -2.5, 0.5, data, "NSIDC Arctic sea ice extent anomaly vs. 1990-2019", filenameArcticExtentAnomaly, 3, True)
	if putOnDropbox:
...
def getPlotMatrix(data, col):
	regional = data[1:,col]
	regional = np.array([i.lstrip() for i in regional]).astype(float)/1000.0
	offset = 245 #275 #306 #334 #31 #61 #92 #122 #153 #184 #214 #275 #61 # 0
...
def printRegionalData(data, ax, col, ymin, ymax, name, legendpos=1, anomaly=False):
	...
	ax.axis([0, 123, ymin, ymax]) #365
	ax.grid(True);
	

	months = ['May','Jun','Jul','Aug']
	#ax.set_xticks([0,30,61,92,120,151], ['', '', '', '', '', ''])
	ax.set_xticks([0,31,61,92,123], ['', '', '', '', '']) #, 211,242,272,303,333,364, '', '', '', '', '', ''])
	#ax.set_xticks([0,31,59,90,120,151,181], ['', '', '', '', '', '', '']) #, 211,242,272,303,333,364, '', '', '', '', '', ''])
	ax.xaxis.set_minor_locator(ticker.FixedLocator([15.5,46,76.5,107.5])) #,196,226.5,257,287.5,318,348.5]))
