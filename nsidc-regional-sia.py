...
def plotRegionalGraphsAntarctic(filename):
	...
	saveRegionalPlot(2, 0, 5.5, data, "Weddell NSIDC sea ice area", "nsidc-area-weddell.png", 2) #6.3
	saveRegionalPlot(3, 0, 6, data, "Weddell NSIDC sea ice extent", "nsidc-extent-weddell.png", 2) # 7.4
...
def plotRegionalGraphsArctic(filename):
	data = np.loadtxt(filename + ".csv", delimiter=",", dtype=str)
	legendpos = 8
	saveRegionalPlot(2, 3.0, 4.6, data, "Central Arctic NSIDC sea ice area", "nsidc-area-cab.png", 3)
	...
	saveRegionalPlot(-3, 0.7, 0.92, data, "NSIDC Arctic sea ice compactness (area divided by extent)", filenameArcticCompactness, 3)
	saveRegionalPlot(-2, 6, 7.3, data, "Arctic Basin NSIDC sea ice extent", filenameArcticBasinExtent, 3)
	saveRegionalPlot(-1, 5, 7.3, data, "Arctic Basin NSIDC sea ice area", filenameArcticBasinArea, 3)
...
def getPlotMatrix(data, col):
	regional = data[1:,col]
	regional = np.array([i.lstrip() for i in regional]).astype(float)/1000.0
	offset = 306 #334 #62 #124 #153 #214 #245 #275 #306 #334 #31 #61 #92 #122 #153 #184 #214 #275 #61 # 0
	years = getNumberOfYears()
