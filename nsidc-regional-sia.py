...
def plotRegionalGraphsAntarctic(filename):
	...
	saveRegionalPlot(3, 0, 7, data, "Weddell NSIDC sea ice extent", "nsidc-extent-weddell.png", 2) # 7.4
	...
	saveRegionalPlot(15, 0, 19, data, "NSIDC Antarctic sea ice extent", filenameAntarcticExtent, 2)
...
def plotRegionalGraphsArctic(filename):
	...
	saveRegionalPlot(-2, 4, 7.3, data, "Arctic Basin NSIDC sea ice extent", filenameArcticBasinExtent, 3)
	saveRegionalPlot(-1, 3, 7.3, data, "Arctic Basin NSIDC sea ice area", filenameArcticBasinArea, 3)
...
def getPlotMatrix(data, col):
	regional = data[1:,col]
	regional = np.array([i.lstrip() for i in regional]).astype(float)/1000.0
	offset = 275 #306 #334 #62 #124 #153 #214 #245 #275 #306 #334 #31 #61 #92 #122 #153 #184 #214 #275 #61 # 0
	years = getNumberOfYears()
