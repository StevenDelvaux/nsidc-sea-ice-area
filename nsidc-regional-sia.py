...
def plotRegionalGraphsAntarctic(filename):
	...
	saveRegionalPlot(2, 0, 6, data, "Weddell NSIDC sea ice area", "nsidc-area-weddell.png", 3) #6.3
	saveRegionalPlot(3, 0, 7, data, "Weddell NSIDC sea ice extent", "nsidc-extent-weddell.png", 3) # 7.4
	saveRegionalPlot(4, 0, 2.2, data, "Bellingshausen-Amundsen NSIDC sea ice area", "nsidc-area-bellamu.png", 1) #2.3
	saveRegionalPlot(5, 0, 2.8, data, "Bellingshausen-Amundsen NSIDC sea ice extent", "nsidc-extent-bellamu.png", 1) #3.4
	saveRegionalPlot(6, 0, 4.0, data, "Ross NSIDC sea ice area", "nsidc-area-ross.png", 1) #4
	saveRegionalPlot(7, 0, 4.8, data, "Ross NSIDC sea ice extent", "nsidc-extent-ross.png", 1) # 5
	saveRegionalPlot(8, 0.0, 1.8, data, "Pacific Southern Ocean NSIDC sea ice area", "nsidc-area-pacific.png", 1) #2
	saveRegionalPlot(9, 0, 2.5, data, "Pacific Southern Ocean NSIDC sea ice extent", "nsidc-extent-pacific.png", 1) #2.4
	saveRegionalPlot(10, 0.0, 3.6, data, "Indian Southern Ocean NSIDC sea ice area", "nsidc-area-indian.png", 1) #3.7
	...
	saveRegionalPlot(14, 0, 16, data, "NSIDC Antarctic sea ice area", filenameAntarcticArea, 1)
	saveRegionalPlot(15, 0, 20, data, "NSIDC Antarctic sea ice extent", filenameAntarcticExtent, 1)
...
def plotRegionalGraphsArctic(filename):
	data = np.loadtxt(filename + ".csv", delimiter=",", dtype=str)
	legendpos = 8
	saveRegionalPlot(2, 2, 4.6, data, "Central Arctic NSIDC sea ice area", "nsidc-area-cab.png", 4)
	saveRegionalPlot(3, 3, 4.6, data, "Central Arctic NSIDC sea ice extent", "nsidc-extent-cab.png", 4)
	...
	saveRegionalPlot(8, 0, 0.75, data, "East Siberian NSIDC sea ice area", "nsidc-area-ess.png", 4)
	saveRegionalPlot(9, 0, 0.75, data, "East Siberian NSIDC sea ice extent", "nsidc-extent-ess.png", 4)
	...
	saveRegionalPlot(18, 0, 1.2, data, "Baffin Bay NSIDC sea ice area", "nsidc-area-baffin.png", 2)
	saveRegionalPlot(19, 0, 1.4, data, "Baffin Bay NSIDC sea ice extent", "nsidc-extent-baffin.png", 2)
	saveRegionalPlot(22, 0, 1.3, data, "Hudson Bay NSIDC sea ice area", "nsidc-area-hudson.png", 2)
	saveRegionalPlot(23, 0, 1.3, data, "Hudson Bay NSIDC sea ice extent", "nsidc-extent-hudson.png", 2)
	saveRegionalPlot(24, 0, 0.85, data, "Canadian Archipelago NSIDC sea ice area", "nsidc-area-caa.png", 4)
	saveRegionalPlot(25, 0, 0.85, data, "Canadian Archipelago NSIDC sea ice extent", "nsidc-extent-caa.png", 4)
	saveRegionalPlot(26, 0, 0.8, data, "Bering NSIDC sea ice area", "nsidc-area-bering.png", 2)
	saveRegionalPlot(27, 0, 1.0, data, "Bering NSIDC sea ice extent", "nsidc-extent-bering.png", 2)
	saveRegionalPlot(28, 0, 0.8, data, "Okhotsk NSIDC sea ice area", "nsidc-area-okhotsk.png", 2)
	saveRegionalPlot(29, 0, 1, data, "Okhotsk NSIDC sea ice extent", "nsidc-extent-okhotsk.png", 2)
	...
	saveRegionalPlot(-3, 0.65, 0.92, data, "NSIDC Arctic sea ice compactness (area divided by extent)", filenameArcticCompactness, 4)
	saveRegionalPlot(-2, 3, 7.4, data, "Arctic Basin NSIDC sea ice extent", filenameArcticBasinExtent, 4)
	saveRegionalPlot(-1, 2.5, 7.4, data, "Arctic Basin NSIDC sea ice area", filenameArcticBasinArea, 4)
	...
	saveRegionalPlot(41, 3.0, 13, data, "NSIDC Arctic sea ice area", filenameArcticArea, 4)
	saveRegionalPlot(42, 4.0, 15, data, "NSIDC Arctic sea ice extent", filenameArcticExtent, 4)	
	saveRegionalPlot(41, -2.5, 0.25, data, "NSIDC Arctic sea ice area anomaly vs. 1990-2019", filenameArcticAreaAnomaly, 2, True)
	saveRegionalPlot(42, -2.75, 0.3, data, "NSIDC Arctic sea ice extent anomaly vs. 1990-2019", filenameArcticExtentAnomaly, 2, True)
	if putOnDropbox:
...
def getPlotMatrix(data, col):
	regional = data[1:,col]
	regional = np.array([i.lstrip() for i in regional]).astype(float)/1000.0
	offset = 92 #122 #153 #184 #214 #275 #61 # 0
	years = 36 if north else 47
	padded = np.pad(regional, (offset, 365*years - regional.shape[0] - offset), 'constant', constant_values=(np.nan,)) #45	
	matrix = padded.reshape((years,365))
	return matrix
...
def printRegionalData(data, ax, col, ymin, ymax, name, legendpos=1, anomaly=False):
	...
	months = ['Oct', 'Nov', 'Dec', 'Jan'] #, 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
